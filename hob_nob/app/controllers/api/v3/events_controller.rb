class Api::V3::EventsController < ApplicationController
  ...
  # caches_action :index, :cache_path => Proc.new { |c| c.params["controller"].to_s+c.params["previous_date_time"].to_s+c.params["mobile_application_code"].to_s+c.params["event_id"].to_s+c.params["is_invitee_group"].to_s }, :expires_in => 2.minutes.to_i
  ...
  def index
    ...
    if mobile_application.present?
      ...
      all_events_listing_data = Event.where(:id => all_events_event_ids).as_json(:only => [:id, :event_name, :cities, :venues, :logo_updated_at, :status, :inside_logo_updated_at, :theme_id, :login_at, :event_category, :marketing_app, :start_event_time, :end_event_time, :description, :about], :methods => [:logo_url,:inside_logo_url,:about_date])
      
      all_updated_events_data = Event.where(:id => all_updated_event_ids).as_json(:only => [:id, :event_name, :cities, :venues, :logo_updated_at, :status, :inside_logo_updated_at, :theme_id, :login_at, :event_category, :marketing_app, :start_event_time, :end_event_time, :description, :about], :methods => [:logo_url,:inside_logo_url,:about_date])
      # params[:event_id].present? ? [] :

      events_having_access = Event.where(:id => allow_ids).as_json(:only => [:id, :event_name, :cities, :venues, :logo_updated_at, :status, :inside_logo_updated_at, :theme_id, :login_at, :event_category, :marketing_app, :start_event_time, :end_event_time, :description, :about], :methods => [:logo_url,:inside_logo_url,:about_date])
      ...
      data = SyncMobileData.sync_records(start_event_date, end_event_date, mobile_application.id, mobile_current_user,submitted_app, event_ids, all_updated_event_ids,hard_refresh)
