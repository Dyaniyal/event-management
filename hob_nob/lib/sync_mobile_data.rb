module SyncMobileData
  ...
  def self.create_record(values, model_name)
    ...
    values.each do |value|
      ...
      if !update_data.blank?
        ...
        message <<  update_data.as_json(except: [:ios_push_response, :android_push_response]) rescue nil
      else
        ...
        message << create_data.as_json(except: [:ios_push_response, :android_push_response]) 
        if model_name == "Qna"
    ...
    model_name.each do |model|
      ...
      case model
        when 'Conversation'
          ...
          #event = Event.where(:id => event_ids)
          #event_ids = (event.present? and event.first.multi_lng_parent_id.present?) ? [event.first.multi_lng_parent_id] : event_ids
          #info = self.get_model_class(model).where(:updated_at => start_event_date..end_event_date).where(:event_id => event_ids) if event_ids.present?
        ...
        when 'EventFeature'
          if current_user.present?
            ...
            data[:"#{name_table(model)}"] = data1.as_json(:only => [:id,:name,:event_id,:page_title,:sequence, :status, :description, :menu_visibilty, :menu_icon_visibility, :session_to_agenda, :main_icon_updated_at, :menu_icon_updated_at, :show_event_feature], :methods => [:main_icon_url, :menu_icon_url])
          elsif feature_visible_flag
            data[:"#{name_table(model)}"] = []
          else
            data[:"#{name_table(model)}"] = info.as_json(:only => [:id,:name,:event_id,:page_title,:sequence, :status, :description, :menu_visibilty, :menu_icon_visibility, :session_to_agenda, :main_icon_updated_at, :menu_icon_updated_at, :show_event_feature], :methods => [:main_icon_url, :menu_icon_url])
          end
        when 'Speaker'
          if current_user.present?
        ...
        when 'Invitee'
          ...
          data[:"all_feedback_forms_last_updated_at"] = current_user.all_feedback_forms_last_updated_at_on_time_basis(nil, nil, all_event_ids,start_event_date,end_event_date)
            ...
            if multilingual_event.present? and multi_lingual_event_id.present?
              invitee_ids = current_user.get_similar_invitees(multi_lingual_event_id).pluck(:id)
            else
        ...
        when 'LogChange'
          if not (start_event_date == "01/01/1990 13:26:58".to_time.utc)
            ...
            # start_event_date = (Time.now.utc - 2.minutes)
            start_event_date = (Time.now.utc - 8.days)
            ...
            # info = LogChange.where('created_at  >?',start_event_date).where(:action => "destroy", :user_id => current_user.present? ? current_user.id : '')
            data[:"#{name_table(model)}"] = info.as_json(:only => [:resourse_type,:resourse_id,:updated_at,:action]) rescue []
          end
        when 'Favorite'
          if current_user.present?
        ...
        when 'UserPoll'
          if current_user.present?
            #event_ids = (current_user.event.present? and current_user.event.multi_lng_parent_id.present?) ? [current_user.event.multi_lng_parent_id] : event_ids
            if multilingual_event.present? and multi_lingual_event_id.present?
              polls = Poll.where(:event_id => multi_lingual_event_id) #if polls.blank?
            else
        ...
        when 'UserQuiz'
          if current_user.present?
            #event_ids = (current_user.event.present? and current_user.event.multi_lng_parent_id.present?) ? [current_user.event.multi_lng_parent_id] : event_ids
            if multilingual_event.present? and multi_lingual_event_id.present?
              quizzes = Quiz.where(:event_id => multi_lingual_event_id) rescue nil #if quizzes.blank?
            else
        ...
        when 'Rating'
          if current_user.present?
            #event_ids = (current_user.event.present? and current_user.event.multi_lng_parent_id.present?) ? [current_user.event.multi_lng_parent_id] : event_ids
            if multilingual_event.present? and multi_lingual_event_id.present?
              speaker_ids = Speaker.where(:event_id => multi_lingual_event_id) rescue nil
              agenda_ids = Agenda.where(:event_id => multi_lingual_event_id) rescue nil
              sponsor_ids = Sponsor.where(:event_id => multi_lingual_event_id) rescue nil
            else
            ...
            info = Rating.where(:ratable_id => [speaker_ids,agenda_ids,sponsor_ids].flatten, :updated_at => start_event_date..end_event_date) rescue []
            data[:"#{name_table(model)}"] = info.as_json() rescue []
          end
        when 'Qna'
          info = info.where(:status => 'approved') if info.present?
        ...
        when 'UserFeedback'  
          if current_user.present?
            #event_ids = (current_user.event.present? and current_user.event.multi_lng_parent_id.present?) ? [current_user.event.multi_lng_parent_id] : event_ids
            if multilingual_event.present? and multi_lingual_event_id.present?
              feedback_ids = Feedback.where(:event_id => multi_lingual_event_id) rescue nil
            else
  ...
  def self.get_feaure_data(info, current_user, speaker_data)
    ...
    info.each do |i|
      if i.send(speaker_data) == "group" 
        ...
        data1 << i if invitee_ids.include? current_user.id.to_s
      else
        data1 << i
      end
    end
  ...
  def self.get_e_kit_feature_data(info, current_user)
    data = []
    value = {}
    info.each do |i|
      value["type"], value["tag"] = i["type"], i["name"]
      value["list"] = i["list"].map{|x| x if x["feature_invitee_ids"].split(",").include? current_user.id.to_s }.compact
      data << value
      value = {}
    end
    data
