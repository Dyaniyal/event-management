...
#  config.logger = Logger.new(STDOUT)
  config.logger = Logger.new('log/production_us.log')
  # Do not dump schema after migrations.
  config.active_record.dump_schema_after_migration = false
  config.action_mailer.default_url_options = { :host => 'us.hobnobspace.com' }
# Settings specified here will take precedence over those in config/application.rb.
...
# :ignore_exceptions => ['ActionView::TemplateError'] + ExceptionNotifier.ignored_exceptions,
 :email => {
   ...
   :exception_recipients => %w{gopikrishna@ascratech.in dyaniyal@ascratech.in}
 }
...
PAPERCLIP_SETTINGS =  {
                        :url => ':s3_alias_url',
                        :s3_protocol => "http",
                        :s3_host_alias => 'd19cruh2nf0cuf.cloudfront.net',
                        :storage => :s3,
